...
- Podman secrets for credentials, injected into containers via the Quadlet
  `Secret=` directive.
...
tofu import podman-quadlet_secret.db secret/db
tofu import podman-quadlet_volume.data volume/service-data
```

Only files previously generated by this provider can be imported; secrets are
adopted by name.
...
still stores them in state. For credentials, prefer a
`podman-quadlet_secret` resource: its value is written over SSH standard input
